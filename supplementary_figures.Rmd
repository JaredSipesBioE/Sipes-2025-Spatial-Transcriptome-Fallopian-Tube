...
f_plot <- volcano_plot(df = df1$amp_v_isth, 
...
wrap_elements(top) / ((a | b | c ) / (d | e | f_plot)) / wrap_elements(bottom) / ((g | h | i ) / (j | k | l)) + plot_layout(heights = c(1,15,1,15), guides = "collect") + plot_annotation(tag_levels = list(c("", "a", "b", "c", "d", "e", "f", "", "g", "h", "i", "j", "k", "l"))) & 
...
```{r fig.height=3.75, fig.width=4.95}
...
fim_v_inf_disc <- volcano_plot_S6(df = df1$fim_v_inf, 
                 title_name = "Fimbria v Infundibulum, Ciliated Cells, Discovery", title_size = title, 
                 upreg = "Fimbria", downreg = "Infundibulum",
                 highlight_genes = mature_cil, flip_log_fold = TRUE, point_size = 3
                 ) |> add_labels(df = df1$fim_v_inf, lab_size = 5, 
                                 highlight_genes = mature_cil, n_genes =0, flip_log_fold = T) |> remove_legend()

inf_v_amp_disc <- volcano_plot_S6(df = df1$inf_v_amp, 
                 title_name = "Infundibulum v Ampulla, Ciliated Cells, Discovery", title_size = title, 
                 upreg = "Infundibulum", downreg = "Ampulla",
                 highlight_genes = mature_cil, flip_log_fold = TRUE, point_size = 3
                 ) |> add_labels(df = df1$inf_v_amp, lab_size = 5, 
                                 highlight_genes = mature_cil, n_genes =0, flip_log_fold = TRUE) |> remove_legend()

amp_v_isth_valid <- volcano_plot_S6(df = df1$amp_v_isth, 
                 title_name = "Ampulla v Isthmus, Ciliated Cells, Discovery", title_size = title, 
                 upreg = "Ampulla", downreg = "Isthmus",
                 highlight_genes = mature_cil, flip_log_fold = TRUE, point_size = 3
                 ) |> add_labels(df = df1$amp_v_isth, lab_size = 5, highlight_genes = mature_cil, n_genes =0, flip_log_fold = T) |> remove_legend()




fim_v_inf_disc + inf_v_amp_disc + amp_v_isth_valid
...
make_fig3 <- function(genes = Fig3, dataset = all_q_norm, title = "All"){
  ...
  legend <- make_boxplot_legend(dataset = dataset)
  ...
  plot <- (Fig3_sig[[1]] + Fig3_sig[[2]]) / legend + 
    plot_layout(heights = c(3, 1)) + 
    plot_annotation(title = title, theme = theme(text=element_text(size=30)))
...
F3_all <- make_fig3(genes = Fig3, dataset = all_q_norm, title = "All")


F3_disc <- make_fig3(genes = Fig3, dataset = disc_q_norm, title = "Discovery")


F3_valid <- make_fig3(genes = Fig3, dataset = valid_q_norm, title = "Validation")
...
figS7_genes <- c("HLA-DPA1", "HLA-DPB1", "HLA-DRA", "HLA-DRB", "HLA-DRB3", "HLA-DRB4", "HLA-DQA1", "HLA-DQB1", "HLA-DMA", "HLA-DMB", "HLA-DOA", "CD74", "C2", "C4B", "CFB")


make_figS7_boxplots <- function(genes = FigS7_genes, dataset = all_q_norm, title = "All"){
...
fS7_disc <- make_figS7_boxplots(genes = figS7_genes , dataset = disc_q_norm)
names(fS7_disc) <- figS7_genes

# Create all of the Boxplots for the Validation Dataset

fS7_valid <- make_figS7_boxplots(genes = figS7_genes , dataset = valid_q_norm)
names(fS7_valid) <- figS7_genes
...
middle <- grobTree(rectGrob(gp=gpar(fill='#F0F0F0',col= 'black')), textGrob('Discovery Dataset: MHC-II Helper transcripts', gp=gpar(fontsize=25)))

bottom <- grobTree(rectGrob(gp=gpar(fill='#F0F0F0',col= 'black')), textGrob('Discovery Dataset: Cytokines in HLA locus', gp=gpar(fontsize=25)))


top_2 <- grobTree(rectGrob(gp=gpar(fill='#F0F0F0',col= 'black')), textGrob('Validation Dataset: MHC-II transcripts', gp=gpar(fontsize=25)))

middle_2 <- grobTree(rectGrob(gp=gpar(fill='#F0F0F0',col= 'black')), textGrob('Validation Dataset: MHC-II Helper transcripts', gp=gpar(fontsize=25)))

bottom_2 <- grobTree(rectGrob(gp=gpar(fill='#F0F0F0',col= 'black')), textGrob('Validation Dataset: Cytokines in HLA locus', gp=gpar(fontsize=25)))
...
legend_valid <- make_boxplot_legend(dataset = valid_q_norm)
  ...
  wrap_elements(top) / 
  (fS7_disc$`HLA-DRB3` | fS7_disc$`HLA-DRB4` |  fS7_disc$`HLA-DPB1` | fS7_disc$`HLA-DQB1`) /
  legend_disc /
  # Part 2 - helper MHCII Transcripts  
  wrap_elements(middle) / 
  (fS7_disc$`HLA-DMA` | fS7_disc$`HLA-DOA` |  fS7_disc$`HLA-DMB` | fS7_disc$`CD74`) /
  legend_disc /
  # Part 3 - Cytokines in HLA Locus 
  wrap_elements(bottom) /
  (fS7_disc$`C2` | fS7_disc$`C4B` |  fS7_disc$`CFB`) /
  legend_disc /
    
  # Part 4 - disc MHCII Transcripts
  wrap_elements(top_2) / 
  (fS7_valid$`HLA-DRB3` | fS7_valid$`HLA-DRB4` |  fS7_valid$`HLA-DPB1` | fS7_valid$`HLA-DQB1`) /
  legend_disc /
  # Part 5 - helper MHCII Transcripts  
  wrap_elements(middle_2) / 
  (fS7_valid$`HLA-DMA` | fS7_valid$`HLA-DOA` |  fS7_valid$`HLA-DMB` | fS7_valid$`CD74`) /
  legend_disc /
  # Part 3 - Cytokines in HLA Locus 
  wrap_elements(bottom_2) /
  (fS7_valid$`C2` | fS7_valid$`C4B` |  fS7_valid$`CFB`) /
  legend_disc /
...
# merge OVGP1 data
pData(all_q_norm) <- pData(all_q_norm) |> left_join(OVGP1_subset |> dplyr::select(region, patient_ID, percent), by = c("region", "patient_ID"))
...
compare_OVGP1 <- function(dataset = all_q_norm, gene = "HLA-DRA", title_append = "Expression v % OVGP1+ Cells"){
...
```{r}
PGR <- compare_OVGP1(gene = "PGR")
ESR1 <- compare_OVGP1(gene = "ESR1")
```
...
bottom <- grobTree(rectGrob(gp=gpar(fill='#F0F0F0',col= 'black')), textGrob('Validation Dataset: Hormone Receptors and Associated Pioneer Transcription Factors', gp=gpar(fontsize=25)))
  ...
  legend_valid /
    (PGR | ESR1)    +
  plot_layout(heights = c(0.5, 4, 0.5, 0.5, 4, 0.5, 4)) 
