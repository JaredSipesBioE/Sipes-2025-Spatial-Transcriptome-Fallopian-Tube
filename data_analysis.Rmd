...
# Create shorter names for anatomical regions

pheno_data <- pheno_data |>
  dplyr::mutate(
    shortRegion = forcats::fct_recode(region, "Amp" = "Ampulla",
                              "Inf" = "Infundibulum",
                              "Fimb" = "Fimbria",
                              "Isth" = "Isthmus"
                              )
...
preQC_all <- preQC_data #preQC_data[, pData(preQC_data)$Patient != "P6"] # to remove Patient 6 data, to test without P6
...
Notes: All 3 datasets end up with the same number of features (1812), so there are no differences in genes that can be analyzed between the three. 
...
Overall, the whole dataset results in two fewer samples being removed compared to discovery and validation separately. 
...
### 6.2 Post Filter Sankey Diagram
...
## 7. Optional: Export Pre-normalized Expression and Annotation Data


#### FUNCT: print_anat_comp_annotations()
...
### 7.1 Export Annotations

```{r}

# 
# print_anat_comp_annotations(target_anat_valid_0.1_genefilter, here::here("Validation_anatomical_comparison.xlsx"))
# 
# print_anat_comp_annotations(target_anat_all_0.1_genefilter, here::here("All_anatomical_comparison.xlsx"))
```


### FUNCT: print_TargetCountMatrix()
...
#                         file_path = here::here("Exprs_Validation_Anatomical_Comparison.xlsx"))
# 
# 
# print_TargetCountMatrix(matrix = exprs(target_anat_all_0.1_genefilter), 
#                         file_path = here::here("Exprs_All_Anatomical_Comparison.xlsx"))
```
